...
callbacks use ``prevent_initial_call="initial_duplicate"``. That only suppresses
the app-load initial batch: a table *inserted* later (router navigation, lazy card
...
def _expand_scoring(page: Page) -> None:
    ...
    # Weights & thresholds are shown by default (review feedback), so no click.
