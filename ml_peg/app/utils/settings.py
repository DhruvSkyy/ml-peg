        ...
        [
            ...
            Div(
                [
                    ...
                    Div(
                        [
                            Div("Font", className="mlpeg-settings-label"),
                            Dropdown(
                                id="font-dropdown",
                                options=[
                                ...
                                value="inter",
                                clearable=False,
                                # Persisted/restored via font-store + sync_font
                                # (single source of truth), mirroring cmap-store;
                                # no Dash persistence so a partial cache clear
                                # can't leave two copies disagreeing.
...
def register_settings_callbacks() -> None:
    ...
    # Apply the stored font choice to <html data-font="..."> (mirrors the theme
    # apply; the no-flash script sets it before first paint). theme.css swaps
    # --font-sans for the "system" value; "inter" keeps the bundled webfont.
    clientside_callback(
        """
        function (font) {
            var f = (font === "system") ? "system" : "inter";
            document.documentElement.setAttribute("data-font", f);
            return "";
        }
        """,
        Output("font-apply-dummy", "children"),
    ...
    def sync_zoom(
        ...
        if trigger_id in (None, "zoom-store"):
            # Clamp on the way in as well: the apply callback and the no-flash
            # script both clamp, so an out-of-range stored value would otherwise
            # leave the slider showing a zoom the tables never render at. bool is
            # an int subclass, so exclude it explicitly.
            valid = isinstance(stored, (int, float)) and not isinstance(stored, bool)
            value = min(ZOOM_MAX, max(ZOOM_MIN, stored)) if valid and stored else 100
    ...
    @callback(
        Output("font-dropdown", "value"),
        Output("font-store", "data", allow_duplicate=True),
        Input("font-dropdown", "value"),
        Input("font-store", "data"),
        prevent_initial_call="initial_duplicate",
    )
    def sync_font(
        ...
        """
        Keep the font dropdown and backing ``font-store`` synchronised.

        Parameters
        ----------
        dropdown_value
            Current dropdown selection ("inter" or "system").
        stored
            Persisted font choice from ``font-store`` (or ``None``).

        Returns
        -------
        tuple[str | object, str | object]
            Dropdown value and store payload; either may be ``dash.no_update``
            depending on which side triggered the sync.
        """
        trigger_id = ctx.triggered_id

        if trigger_id in (None, "font-store"):
            return (stored if stored in ("inter", "system") else "inter"), no_update
        if trigger_id == "font-dropdown":
            return no_update, dropdown_value
        raise PreventUpdate
