...
# Benchmark cards open by default on a category/framework page: enough to show
# some data at a glance without rendering every heavy body up front (review
# feedback — a sweet spot between "first only" and "expand all").
CARDS_OPEN = 2
...
def build_benchmark_card(name: str, layout: object, open_default: bool) -> Details:
    """
    Build a collapsible benchmark card using a native ``<details>``.

    The benchmark body (table + plot + scoring controls) is always rendered, but a
    collapsed ``<details>`` hides its content (``display:none``), so closed cards
    are not laid out or painted. ``open_default`` sets which cards start expanded.
    Collapse is native (no callback), keeping category/framework pages
    interaction-light; the server lazy-mount speed optimisation is a later slice.

    Kept here (rather than in ``build_app``) so both category and framework pages
    can build cards without a circular import.
    ...
    return Details(
        [
            Summary(
                ...
                className="mlpeg-bench-header",
            ),
            Div(layout, className="mlpeg-bench-body"),
        ],
        open=open_default,
    ...
    return Div(
        [
            H2(
                ...
                style={"color": "var(--mlpeg-ink)", "marginTop": "30px"},
            ),
            Div(faq_components),
        ]
    )
...
def build_test_layout(
    ...
    layout_contents = [
        Div(
            [
                H2(name, style={"color": "var(--mlpeg-ink)", "margin": "0"}),
                *[
    ...
    # glance). The cost is bounded because the controls live inside a benchmark
    # card body, which is itself only mounted when the card is expanded.
    ...
    if extra_components:
        # Interactive-zone max-width (#720) around the plot / structure viewers.
        # (The click-to-view loading overlay is deferred to the interactive slice
        # that also fixes the struct-below-plot rendering.)
        layout_contents.append(
