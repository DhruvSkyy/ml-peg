...
def build_framework_page_layout(
    ...
    # Wrap each benchmark in a native collapsible card. The first couple of cards
    # on the page open by default (some data at a glance; review feedback), the
    # rest collapse. ``expand_all`` opens them all when the preference is set.
    sections = []
    card_index = 0
    for category_name, tests in benchmarks_by_category.items():
        ...
        for name, layout in tests:
            cards.append(
                build_benchmark_card(
                    name, layout, open_default=expand_all or card_index < CARDS_OPEN
                )
            )
            card_index += 1
        sections.append(Div(cards, style={"display": "grid", "gap": "24px"}))
